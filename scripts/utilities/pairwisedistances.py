...
import pandas as pd
def pairwise_dist(sel1, sel2, max_dist, output="S", sidechain="N", show="N"):
    """
    ...
    if sidechain=="Y":
        ...
        #builds models
    if sel1==sel2:
        m1 = cmd.get_model(sel1+extra)
        m1o = cmd.get_object_list(sel1)
        m2 = cmd.get_model(sel1+extra)
        m2o = cmd.get_object_list(sel1)
    else:
    ...
    df_index = ['_'.join([m1o[0],m1.atom[c1].chain,m1.atom[c1].resn,m1.atom[c1].resi,m1.atom[c1].name]) for c1 in range(len(m1.atom))]
    df_column = ['_'.join([m2o[0],m2.atom[c2].chain,m2.atom[c2].resn,m2.atom[c2].resi,m2.atom[c2].name]) for c2 in range(len(m2.atom))]
    dist_df = pd.DataFrame(dist_matrix,index=df_index,columns=df_column)

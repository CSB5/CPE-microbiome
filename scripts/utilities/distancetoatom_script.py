...
ARGUMENTS
    ...
    cmd.select(tempsel, '(%s around %f) and (%s) and state %d' %
    ...
    stored.temp = []
    cmd.iterate(
        tempsel, 'stored.temp.append([model, segi, chain, resn, resi, name, alt])')
    ...
    if sort > 0:
        distance_list.sort(key=lambda dist: dist[4])
    elif sort < 0:
        distance_list.sort(key=lambda dist: dist[4], reverse=True)
    # add header
...
cmd.auto_arg[2]['distancetoatom'] = [lambda: cmd.Shortcut(
    ['None', 'distancetoatom_report.txt']), 'filename=', ',']
cmd.auto_arg[3]['distancetoatom'] = [lambda: cmd.Shortcut(
    ['selection=all, state=0, property_name=p.dist, coordinates=0, decimals=3, sort=1']), 'remaining (defaults)...', '']
################################################################################
